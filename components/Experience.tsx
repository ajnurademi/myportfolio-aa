    ...
    return (
        <div className="py-20" id="projects">
            <h1 className="heading text-white">
                My{" "}
                <span className="text-purple">diverse Experiences</span>
            </h1>
            <div className="w-full mt-16 grid lg:grid-cols-4 grid-cols-1 gap-10">
                {workExperience.map((card) => (
                    <Button key={card.id} duration={Math.floor(Math.random() * 10000 + 10000)} borderRadius='1.75rem' className='flex-1 text-white border-neutral-200 dark:border-slate-800'>
                        <div className="flex lg:flex-row flex-col lg:items-center p-3 py-6 md:p-5 lg:p-10 gap-2 sm:w-[34rem] w-[33rem]">
